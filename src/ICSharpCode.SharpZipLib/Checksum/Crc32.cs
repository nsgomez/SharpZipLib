...
namespace ICSharpCode.SharpZipLib.Checksum
{
	...
	public sealed class Crc32 : IChecksum
	{
		...
		private unsafe void Update(byte[] data, int offset, int count)
		{
			...
			{
				byte* pData = pFixedData + offset;
				byte* pDataEnd = pFixedData + end;

				while (pData != pDataEnd)
				{
					checkValue = CrcUtilities.UpdateDataForReversedPoly(pData, pCrcTable, checkValue);
					pData += CrcUtilities.SlicingDegree;
				}

				if (remainder != 0)
				{
					SlowUpdateLoop(pData, remainder);
				}
			}
		}
		...
		/// performance overall.
		/// </summary>
		/// <param name="data">Pointer to the data to add</param>
		/// <param name="count">Number of bytes to process</param>
		[MethodImpl(MethodImplOptions.NoInlining)]
		private unsafe void SlowUpdateLoop(byte* data, int count)
		{
			while (count-- != 0)
			{
				Update(*data++);
